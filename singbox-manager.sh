...
main(){ root_check; os_detect; load; case "${1:-}" in install) packages; install_binary; service_create ;; nodes|node) node_menu ;; anytls) cfg_anytls ;; tuic) cfg_tuic ;; hy2) cfg_hy2 ;; vless-ws|vless_ws) cfg_vless_ws ;; vmess-ws|vmess_ws|vmess) cfg_vmess_ws ;; reality) cfg_reality ;; cert) cert_menu ;; links) links ;; restart) service_restart ;; logs) tail -n 100 "$LOG" 2>/dev/null || true ;; status) status_nodes ;; apply) load; apply_config ;; regen|regen-random) regen_random_current ;; *) menu ;; esac; }
main "$@"